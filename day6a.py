...
def questionsAnswered(letters):
    i = {}
    for letter in letters:
        i[letter] = True
    return i

def processGroups(data):
    data = data.split('\n\n')
    i = {}
    for groupID, groupData in zip(range(0, len(data)), data):
        i[groupID] = groupData.split('\n')
    return i
...
def processQuestions(data):
    ...
    for groupID, groupData in data.items():
        i[groupID] = {}
        for userID, userData in zip(range(0, len(groupData)), groupData):
            i[groupID][userID] = questionsAnswered(letters=userData)
    return i

def checkQuestions(data):
    i = {}
    for groupID, userData in data.items():
        i[groupID] = []
        for userID, questionData in userData.items():
            for question, answer in questionData.items():
                if answer and question not in i[groupID]:
                    i[groupID].append(question)
    return i

def sumQuestions(data):
    i = [len(y) for x,y in data.items()]
    return sum(i)

if __name__ == '__main__':
    data = processGroups(data=data)
    data = processQuestions(data=data)
    data = checkQuestions(data=data)
    data = sumQuestions(data=data)
    print(data)