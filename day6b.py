...
def checkQuestions(data):
    ...
    for groupID, groupData in data.items():
        g = {}
        for userID, userData in groupData.items():
            for question, answer in userData.items():
                if question not in g.keys():
                    g[question] = 0
                if answer:
                    g[question] += 1
        for question, count in g.items():
            if count == len(groupData.keys()):
                if question not in i.keys():
                    i[question] = []
                i[question].append(groupID)
    for k in i.keys():
        i[k] = len(i[k])
    return i

def sumQuestions(data):
    i = [y for x,y in data.items()]
    return sum(i)
