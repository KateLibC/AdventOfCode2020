data = data = '''6-10 p: ctpppjmdpppppp
...
def passwordPolicy(line):
    ...
    rval = rval.split('-')
    rval = [int(x) - 1 for x in rval]
    return rval, cval

def passwordExtract(line):
    return line.split(': ')[-1]

def passwordCheck(policy, password):
    char = policy[1]
    pos0, pos1 = policy[0]
    p1 = password[pos0]
    p2 = password[pos1]
    o = False
    if p1 != p2:
        o = True in [char in [p1, p2]]
    return o

i = {}
for line in data.split('\n'):
    policy = passwordPolicy(line=line)
    password = passwordExtract(line=line)
    check = passwordCheck(policy=policy, password=password)
    if check not in i.keys():
        i[check] = 0
    i[check] += 1
print(i)