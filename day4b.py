...
def parseData(data):
    ...
    data = [dict([y.split(':') for y in x]) for x in data]
    return data

def passportCodes(fields, p):
    i = {}
    for field in fields.keys():
        i[field] = field in p.keys()
    return i

def passportFields(p, optional=None):
    if optional is not None:
        for field in optional:
            if field in p.keys():
                del p[field]
    s = [y for x,y in p.items()]
    return False not in s

def passportCheck(expected, p):
    def checkLength(v, rule):
        rule = rule['l']
        o = True
        if rule is not None:
            o = len(v) == rule
        return o
    def checkRange(v, rule):
        rule = rule['r']
        o = True
        if rule is not None:
            o = int(v) in rule
        return o
    def checkParse(v, rule):
        rule = rule['f']
        o = True
        if rule is not None:
            o = rule(v=v)
    ...
    for k in p.keys():
        ...
        p[k] = i
    p['_rules'] = []
    rr = {'l': checkLength, 'r': checkRange, 'f': checkParse}
    for rule, rulec in expected.items():
        if rule in p.keys():
            v = p[rule]['v']
            n = []
            for rk, rf in rr.items():
                oc = rf(rule=rulec, v=v)
                p[rule]['b'][rk] = oc
                n.append(oc)
            p['_rules'] = p['_rules'] + n

    p['_valid'] = False not in p['_rules']
    return p

def displayPassportDetails(passport):
    validation = {True: ' VALID ', False: 'INVALID'}
    if not passport["_valid"]:
        reasons = []
        for p,v in passport.items():
            if '_' not in p:
                ov = v['v']
                b = v['b']
                cr = []
                if not b['l']:
                    cr.append('Invalid variable length')
                if not b['r']:
                    cr.append('Number not in range')
                if not b['f']:
                    cr.append('Does not meet format')
                if len(cr) != 0:
                    s = f'{p}\t{ov}\tReasons: ' + ', '.join(cr)
                    reasons.append(s)
        reasons = '\n' + '\n'.join(reasons) + '\n'
    else:
        reasons = f'Passport with ID {passport["pid"]} has been verified.'
    o = f'[{validation[passport["_valid"]]}] - {reasons}'
    print(o)
...
def checkHeight(v):
    ...
    if len(v) in [4, 5]:
        ...
        if u.lower() in ['cm', 'in']:
            r = None
            if u == 'cm' and v[:-2].isdigit():
                r = range(150, 193+1)
            if u == 'in' and v[:-2].isdigit():
                r = range(59, 76+1)
            if r is not None:
                o = int(v[:-2]) in r
    return o
...
def checkEyes(v):
    colours = 'amb blu brn gry grn hzl oth'.split()
    v = v.lower()
    return v in colours
...
def checkHair(v):
    ...
    if v[:1] == '#':
        v = v[1:].lower()
        o = False not in [x in 'abcdef0123456789' for x in v]
    return o
...
expected = {
            'byr': {'r': range(1920, 2002+1), 'l': 4, 'f': None}, 
            'iyr': {'r': range(2010, 2020+1), 'l': 4, 'f': None}, 
            'eyr': {'r': range(2020, 2030+1), 'l': 4, 'f': None}, 
            'hgt': {'r': None, 'l': None, 'f': checkHeight}, 
            'hcl': {'r': None, 'l': 7, 'f': checkHair}, 
            'ecl': {'r': None, 'l': 3, 'f': checkEyes}, 
            'pid': {'r': None, 'l': 9, 'f': checkPID}, 
            'cid': {'r': None, 'l': None, 'f': None},
            }

if __name__ == '__main__':
    passports = parseData(data=data)
    valid_passports = 0
    for passport in passports:
        state = passportCodes(fields=expected, p=passport)
        valid = passportFields(p=state, optional=optional)
        if valid:
            check = passportCheck(expected=expected, p=passport)
            if check['_valid']:
                valid_passports += 1
            else:
                displayPassportDetails(passport=check)
