...
def passwordPolicy(line):
    line = line.split(': ')
    rval, cval = line[0].split()
    rval = rval.split('-')
    rval = [int(x) for x in rval]
    rval = range(rval[0], rval[1] + 1)
    return rval, cval

def passwordRetrieve(line):
    return line.split(': ')[-1]

def passwordValid(policy, password):
    s = {}
    for c in password:
        if c not in s.keys():
            s[c] = 0
        s[c] += 1
    o = False
    if policy[1] in s.keys():
        o = s[policy[1]] in policy[0]
    return o

i = {}
for line in data.split('\n'):
    policy = passwordPolicy(line=line)
    password = passwordRetrieve(line=line)
    valid = passwordValid(policy=policy, password=password)
    if valid not in i.keys():
        i[valid] = 0
    i[valid] += 1
print(i)